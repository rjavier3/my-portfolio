...
const displayCurrentPriceChart = data => {
  const ctx = document.getElementById('stockChart').getContext('2d'); // destroy the chart so underlying data isn't shown.
...
const displayHistoryPriceChart = data => {
  const ctx = document.getElementById('stockHistoryChart').getContext('2d'); // destroy the chart so underlying data isn't shown.
...
document.querySelector('.frm.symbol').addEventListener('submit', event => {
  event.preventDefault();
  symbol = event.target.elements.symbol.value;
  fetchCurrentPrice(new Stock({
    symbol: symbol
  }));
}); // add the view history listener

document.querySelector('#view-history-button').addEventListener('click', event => {
  fetchHistory(new Stock({
    symbol: symbol
  }));
});